...
class PermCheckSpec extends FunSpec {
  ...
  describe("PermCheck isPermutation") {
    ...
    it("returns 0 for big array of 100,000 elements all the same values equal to 100,000") {
      val array: Array[Int] = (1 to 100000).map(e => 100000).toArray
      PermCheck.isPermutation(array) shouldBe 0
    }

    it("returns 0 when total sum is correct and it is not permutation for [4, 1, 4, 1]") {
      PermCheck.isPermutation(Array(4, 1, 4, 1)) shouldBe 0
    }

    it("returns 0 when total sum is correct and it is not permutation for big array") {
