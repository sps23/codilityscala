...
object NumberOfDiscIntersections {
  ...
  private[lesson6] val MAX = 10000000

  def numberOfIntersections(a: Array[Int]): Int = {
    @tailrec
    def iterate(aList: List[(Int, Long)], tList: List[Long], n: Int): Int =
      aList match {
        case List() => n
        case h :: t =>
    ...
    val intervalsSortedByStart =
      a.zipWithIndex.map(t => (t._2 - t._1, t._2.toLong + t._1)).sortBy(_._1)
